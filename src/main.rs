...
use matrix_sdk::{
    ruma::{ DeviceIdBox, UserId, RoomId },
    room::Joined as JoinedRoom,
    uuid::Uuid,
    instant::{Instant, Duration},
    Client, ClientConfig, Session, SyncSettings, LoopCtrl,
};
use url::Url;
...
async fn run(args: &Run) -> Result<()> {
    let client = mtx::start()
        .await.context("Matrix startup")?;
    let channel = mtx::channel(args, &client)
        .await.context("Join channel")?;

    let syncclient = client.clone(); // internally all Arcs
    let sync = tokio::spawn(async move { 
        syncclient.sync_with_callback(SyncSettings::default(), |response| async move {
            trace!(?response);
